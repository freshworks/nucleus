...
let sampleActionTemplate = hbs`
  {{nucleus-datepicker onUpdate=(action "onUpdate") onCancel=(action "onCancel")}}
`;

let samplePropTemplate = hbs`
  {{nucleus-datepicker initialDate="1 Apr, 2020"}}
`;
...
module('Integration | Component | nucleus-datepicker', function(hooks) {
  ...
  test('it should highlight date when a date is clicked', async function(assert) {
    await render(sampleTemplate);
    await click('.nucleus-datepicker .ember-power-calendar-day--today');
    await assert.dom('.nucleus-datepicker .ember-power-calendar-day--selected.ember-power-calendar-day--today').exists({ count: 1 }, 'Current date is selected on click');
  });

  test('it should update the input field when a date is clicked', async function(assert) {
    await render(sampleTemplate);
    await click('.nucleus-datepicker .ember-power-calendar-day--today');
    let parsedDate = parseDate(document.querySelector('.ember-power-calendar-day--selected').dataset.date, 'yyyy-MM-dd', 'en');
    let formattedDate = formatDate(parsedDate, 'd MMM, yyyy', 'en');
    await assert.equal(
      document.querySelector('.nucleus-datepicker input').value, 
      formattedDate, 
      'Input field is updated after date is clicked'
    );
  });

  test('it should update calendar when input value is updated', async function(assert) {
    await render(sampleTemplate);
    await click('.nucleus-datepicker .ember-power-calendar-day--today');
    let parsedDate = parseDate(document.querySelector('.ember-power-calendar-day--selected').dataset.date, 'yyyy-MM-dd', 'en');
    let afterDay = add(parsedDate, 1, 'day');
    let formattedDate = formatDate(afterDay, 'd MMM, yyyy', 'en');
    let formattedDateSelector = formatDate(afterDay, 'yyyy-MM-dd', 'en');
    document.querySelector('.nucleus-datepicker input').value = formattedDate;
    document.querySelector('.nucleus-datepicker input').dispatchEvent(new Event('change', { bubbles: true }));
    assert.dom(`.nucleus-datepicker .ember-power-calendar-day--selected[data-date="${formattedDateSelector}"]`).exists({ count : 1 }, 'Changed date selected in clanedar')
  });
